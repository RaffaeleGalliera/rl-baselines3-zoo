...
class HistoryWrapperDiscreteActions(gym.Wrapper):
    """
       Stack past observations and actions to give an history to the agent.
    ...
    def __init__(self, env: gym.Env, horizon: int = 2):
        ...
        low_action = np.repeat(0, horizon, axis=-1)
        high_action = np.repeat(wrapped_action_space.n - 1, horizon, axis=-1)

        low = np.concatenate((low_obs, low_action))
        high = np.concatenate((high_obs, high_action))

        # Overwrite the observation space
        env.observation_space = gym.spaces.Box(low=low, high=high,
    ...
    def step(self, action):
        ...
        self.obs_history = np.roll(self.obs_history, shift=-last_ax_size,
                                   axis=-1)
        self.obs_history[..., -obs.shape[-1]:] = obs
